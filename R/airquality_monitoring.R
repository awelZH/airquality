...
### read and restructure NABEL h1 O3 data from Dübendorf-EMPA and Zürich-Kaserne and calculate WHO'21 metric O3 peak-season
data <-
  fs::path("data/input", files$airquality$monitoring$nabel_due_h1) %>%
  read_nabel_ts_h1() %>%
  dplyr::bind_rows(read_nabel_ts_h1(fs::path("data/input", files$airquality$monitoring$nabel_kas_h1))) %>%
  calc_O3_peakseason() %>%
  dplyr::mutate(source = factor("NABEL (BAFU & Empa)")) %>%
  dplyr::bind_rows(data)
...
### read and restructure OSTLUFT h1 O3 data and calculate WHO'21 metric O3 peak-season
data <-
  fs::path("data/input", files$airquality$monitoring$ostluft_h1) %>%
  read_airmo_csv2() %>%
  calc_O3_peakseason() %>%
