...
# data_rsd_per_norm %>%
#   ggplot2::ggplot(aes(x = vehicle_euronorm, y = n, fill = vehicle_type)) +
#   geom_bar(stat = "identity", color = NA, position = position_dodge()) + 
#   facet_wrap(vehicle_fuel_type~., nrow = 1)

# data_rsd %>% 
#   ggplot2::ggplot(aes(x = vehicle_specific_power, fill = vehicle_euronorm)) +
#   geom_histogram()
