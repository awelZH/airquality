# FIXME: muss der filter wirklich als variable eingegeben werdne? ist der nicht fix?
prep_emmissions <- function(data, filter_args = canton == 'ZH' & emission != 0 & !(subsector %in% c('Weitere Punktquellen OL', 'Rheinschifffahrt', 'Flugverkehr Genf'))){
...
prep_rsd <- function(data, rsd_meta){
  rsd_filters <- read_local_csv("misc_rsd_filters.csv")
  ...
  data_vsp <- calculate_vehicle_specific_power(data)
