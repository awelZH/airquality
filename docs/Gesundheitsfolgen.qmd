...
plots <- 
  plots_outcomes |> 
  dplyr::arrange(parameter, plot) |>
  dplyr::mutate(
    id = 1:dplyr::n(),
    year = dplyr::case_when(plot == "preliminary_deaths_abs" ~ "absolut", TRUE ~ "relativ"),
    markdown = build_panel(id, year, parameter, plot, "outcomes")
  )
```
...
`r knitr::knit(text = as.character(dplyr::filter(plots, parameter == "PM2.5" & stringr::str_detect(plot, "preliminary_deaths"))$markdown))`
...
`r knitr::knit(text = as.character(dplyr::filter(plots, parameter == "NO2" & stringr::str_detect(plot, "preliminary_deaths"))$markdown))`
...
`r knitr::knit(text = as.character(dplyr::filter(plots, parameter == "O3_peakseason_mean_d1_max_mean_h8gl" & stringr::str_detect(plot, "preliminary_deaths"))$markdown))`
