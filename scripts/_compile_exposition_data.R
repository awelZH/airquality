...
years <- get_years(read_all_raster, lubridate::year(Sys.Date()) - 2, base_scenario_year)
...
if ((length(years$all) == 1 & !years$base_analysed) | length(years$all) > 1) {
  ...
  data_raster_aq <- read_all_raster_data(ressources, years, map_canton)
  data_raster_ndep <- read_bafu_raster_data(filter_ressources(ressources, 19), years_filter = years$ndep_exmax, map_canton) # ndep is already exposition data and exists from 1990 on
  years$all <- as.numeric(names(data_raster_aq))
  
  # => download / read BFS statpop data for same years as pollutant raster data
  data_raster_bfs <- purrr::map(setNames(years$all, years$all), function(year) read_statpop_raster_data(year, "inst/extdata", map_canton))
  ...
  data_expo_pop <- prepare_exposition(data_raster_bfs, data_raster_aq, years$all)
  ...
  # ---
  write_local_csv(data_pop_weighted_mean$canton, file = "inst/extdata/output/data_exposition_weighted_means_canton.csv", append = !read_all_raster)
  write_local_csv(data_pop_weighted_mean$munipalities, file = "inst/extdata/output/data_exposition_weighted_means_municipalities.csv", append = !read_all_raster)
  write_local_csv(data_expo_population_dist, file = "inst/extdata/output/data_exposition_distribution_pollutants.csv", append = !read_all_raster)
  if (length(data_raster_ndep) > 0) {write_local_csv(data_expo_ecosys_dist, file = "inst/extdata/output/data_exposition_distribution_ndep.csv", append = !read_all_raster)}
